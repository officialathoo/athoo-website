...
async function sendSmtpEmail(opts: { to: string; subject: string; html: string; replyTo?: string }) {
  if (!isSmtpConfigured()) return { sent: false, skipped: true, reason: "smtp_not_configured" };
...
async function ensureSchema() {
  ...
  await db.query(
    ...
    ["Super Admin", process.env.LEAD_NOTIFY_TO || "official@athoo.pk", JSON.stringify({ all: true })]
  );
}
