...
import {
  ...
  BookOpen, FileText, Pencil, Trash2, Globe, EyeOff,
} from "lucide-react";
...
export default function Admin() {
  ...
  const [activeTab, setActiveTab] = useState<"leads" | "email" | "admins" | "settings" | "activity" | "blogs" | "templates">("leads");
  ...
  type BlogDraft = { id: number; title: string; slug: string; category: string; excerpt: string; content: string; author: string; status: "draft" | "published"; publishedAt: string; };
  const defaultTemplates = [
    { id: 1, name: "Launch Notification", subject: "Athoo Is Launching in Rawalpindi & Islamabad!", body: "Hi {{name}},\n\nGreat news! Athoo is launching soon in Rawalpindi and Islamabad. You're on our waitlist and will be among the first to know when we go live.\n\nStay tuned for updates!\n\nThe Athoo Team\nsupport@athoo.pk" },
    { id: 2, name: "Provider Onboarding", subject: "Your Athoo Provider Registration Has Been Received", body: "Hi {{name}},\n\nThank you for registering as a service provider on Athoo. Our team has received your interest form.\n\nWe will contact you at {{phone}} when provider onboarding officially opens to guide you through the verification process.\n\nIf you have any questions, reach us at official@athoo.pk or WhatsApp: +92 339 0051068.\n\nRegards,\nAthoo Team" },
    { id: 3, name: "Provider Approved", subject: "You're Approved as an Athoo Provider!", body: "Hi {{name}},\n\nCongratulations! Your application to join Athoo as a {{service}} provider has been approved.\n\nYou will receive further instructions to set up your provider profile and start receiving job requests when the app launches.\n\nWelcome to the Athoo network!\n\nAthoo Team" },
    { id: 4, name: "Inquiry Response", subject: "Re: Your Athoo Inquiry", body: "Hi {{name}},\n\nThank you for reaching out to Athoo. We've received your message and our team will respond within 1–2 business days.\n\nFor urgent matters, you can also reach us on WhatsApp at +92 339 0051068.\n\nRegards,\nAthoo Support\nsupport@athoo.pk" },
    { id: 5, name: "Waitlist Confirmation", subject: "You're on the Athoo Waitlist!", body: "Hi {{name}},\n\nWelcome to the Athoo waitlist! You've successfully joined and will be among the first to know when Athoo launches in Rawalpindi and Islamabad.\n\nFollow us on Instagram @athoo_services for the latest updates.\n\nThe Athoo Team" },
  ];
  const [blogs, setBlogs] = useState<BlogDraft[]>([]);
  const [blogForm, setBlogForm] = useState<Partial<BlogDraft>>({ title: "", slug: "", category: "Insights", excerpt: "", content: "", author: "Athoo Team", status: "draft", publishedAt: new Date().toISOString().slice(0, 10) });
  ...
  function saveBlog() {
    ...
    if (editingBlog !== null) {
      setBlogs(blogs.map((b) => b.id === editingBlog ? { ...b, ...blogForm } as BlogDraft : b));
      setNotice("Blog updated."); setEditingBlog(null);
    } else {
      const newBlog: BlogDraft = { ...blogForm, id: Date.now() } as BlogDraft;
      setBlogs([...blogs, newBlog]);
      setNotice("Blog draft created.");
    }
    setBlogForm({ title: "", slug: "", category: "Insights", excerpt: "", content: "", author: "Athoo Team", status: "draft", publishedAt: new Date().toISOString().slice(0, 10) });
  }

  function deleteBlog(id: number) { setBlogs(blogs.filter((b) => b.id !== id)); setNotice("Blog removed."); }
  function editBlog(b: BlogDraft) { setBlogForm(b); setEditingBlog(b.id); }
        ...
        <div className="mx-auto flex max-w-[1600px] flex-col gap-5 p-3 sm:p-5 lg:flex-row">
          <aside className="rounded-[1.75rem] bg-[#081120] p-4 text-white shadow-xl lg:sticky lg:top-5 lg:h-[calc(100vh-40px)] lg:w-72">
            ...
            <nav className="grid grid-cols-2 gap-2 lg:grid-cols-1">
              {[
                ["leads", Users, "Leads CRM"], ["email", Mail, "Bulk Email"], ["blogs", BookOpen, "Blog CMS"], ["templates", FileText, "Email Templates"], ["admins", UserCog, "Admins"], ["settings", Wrench, "Settings"], ["activity", BarChart3, "Activity"],
          ...
          <section className="min-w-0 flex-1">
            ...
            {activeTab === "blogs" && <div className="grid gap-5 xl:grid-cols-[420px_1fr]">
              <div className="rounded-[1.75rem] bg-white p-5 shadow-sm">
                <h2 className="mb-4 text-2xl font-black"><BookOpen className="mr-2 inline h-5 w-5 text-[#0057FF]" />{editingBlog !== null ? "Edit Blog" : "New Blog Post"}</h2>
                <p className="mb-4 text-xs text-gray-400">Blog CMS is local state. Connect /api/admin/blogs to persist posts.</p>
                <Input className="mb-2 min-h-11" placeholder="Title" value={blogForm.title || ""} onChange={(e) => setBlogForm({ ...blogForm, title: e.target.value, slug: blogForm.slug || e.target.value.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "") })} />
                <Input className="mb-2 min-h-11" placeholder="Slug (url-friendly)" value={blogForm.slug || ""} onChange={(e) => setBlogForm({ ...blogForm, slug: e.target.value })} />
                <Input className="mb-2 min-h-11" placeholder="Category" value={blogForm.category || ""} onChange={(e) => setBlogForm({ ...blogForm, category: e.target.value })} />
                <Input className="mb-2 min-h-11" placeholder="Author" value={blogForm.author || ""} onChange={(e) => setBlogForm({ ...blogForm, author: e.target.value })} />
                <Input className="mb-2 min-h-11" type="date" value={blogForm.publishedAt || ""} onChange={(e) => setBlogForm({ ...blogForm, publishedAt: e.target.value })} />
                <textarea className="mb-2 w-full rounded-xl border p-3 min-h-[80px] text-sm" placeholder="Excerpt (short summary)" value={blogForm.excerpt || ""} onChange={(e) => setBlogForm({ ...blogForm, excerpt: e.target.value })} />
                <textarea className="mb-3 w-full rounded-xl border p-3 min-h-[200px] text-sm font-mono" placeholder="Content (HTML or markdown)" value={blogForm.content || ""} onChange={(e) => setBlogForm({ ...blogForm, content: e.target.value })} />
                <div className="mb-3 flex items-center gap-3">
                  <label className="text-sm font-semibold">Status:</label>
                  <select className="rounded-xl border px-3 py-2 text-sm" value={blogForm.status || "draft"} onChange={(e) => setBlogForm({ ...blogForm, status: e.target.value as "draft" | "published" })}>
                    <option value="draft">Draft</option>
                    <option value="published">Published</option>
                  </select>
                </div>
                <div className="flex gap-2">
                  <Button className="bg-[#0057FF] flex-1" onClick={saveBlog}><Pencil className="mr-2 h-4 w-4" />{editingBlog !== null ? "Update" : "Create Post"}</Button>
                  {editingBlog !== null && <Button variant="secondary" onClick={() => { setEditingBlog(null); setBlogForm({ title: "", slug: "", category: "Insights", excerpt: "", content: "", author: "Athoo Team", status: "draft", publishedAt: new Date().toISOString().slice(0, 10) }); }}>Cancel</Button>}
                </div>
              </div>
              <div className="rounded-[1.75rem] bg-white p-5 shadow-sm">
                <h2 className="mb-4 text-2xl font-black">Blog Posts ({blogs.length})</h2>
                <div className="grid gap-3">
                  {blogs.map((b) => <div key={b.id} className="rounded-2xl border p-4">
                    <div className="flex items-start justify-between gap-2">
                      <div className="flex-1">
                        <div className="font-black">{b.title}</div>
                        <div className="text-xs text-gray-500 mt-1">{b.slug} • {b.category} • {b.author} • {b.publishedAt}</div>
                        <div className="mt-2 flex gap-2">
                          <span className={`inline-flex items-center gap-1 rounded-full px-3 py-0.5 text-xs font-bold ${b.status === "published" ? "bg-green-50 text-green-700" : "bg-gray-100 text-gray-500"}`}>{b.status === "published" ? <Globe className="h-3 w-3" /> : <EyeOff className="h-3 w-3" />}{b.status}</span>
                        </div>
                      </div>
                  ...
                  {!blogs.length && <div className="rounded-2xl bg-gray-50 border border-dashed border-gray-200 p-8 text-center text-sm text-gray-500"><BookOpen className="h-8 w-8 mx-auto mb-3 text-gray-300" /><p className="font-semibold mb-1">No blog posts yet</p><p>Create your first post using the form on the left.</p></div>}
                </div>
              </div>
            </div>}
            ...
            {activeTab === "templates" && <div className="grid gap-5 xl:grid-cols-[420px_1fr]">
              <div className="rounded-[1.75rem] bg-white p-5 shadow-sm">
                <h2 className="mb-4 text-2xl font-black"><FileText className="mr-2 inline h-5 w-5 text-[#0057FF]" />{editingTemplate !== null ? "Edit Template" : "New Template"}</h2>
                <p className="mb-4 text-xs text-gray-400">Use {"{{name}}"}, {"{{phone}}"}, {"{{service}}"} as placeholders. Templates integrate with Bulk Email tab.</p>
                ...
                <textarea className="mb-3 w-full rounded-xl border p-3 min-h-[260px] text-sm font-mono" placeholder="Email body text" value={templateForm.body} onChange={(e) => setTemplateForm({ ...templateForm, body: e.target.value })} />
                <div className="flex gap-2">
                  <Button className="bg-[#0057FF] flex-1" onClick={saveTemplate}>{editingTemplate !== null ? "Update Template" : "Save Template"}</Button>
              ...
              <div className="rounded-[1.75rem] bg-white p-5 shadow-sm">
                <h2 className="mb-4 text-2xl font-black">Email Templates ({templates.length})</h2>
                <div className="grid gap-3">
                  {templates.map((t) => <div key={t.id} className="rounded-2xl border p-4">
                    <div className="flex items-start justify-between gap-2">
                      <div className="flex-1">
                        <div className="font-black">{t.name}</div>
                        <div className="text-xs text-gray-500 mt-1 font-medium">{t.subject}</div>
                        <div className="mt-2 text-xs text-gray-400 line-clamp-2 font-mono">{t.body.slice(0, 100)}...</div>
                      </div>
                      <div className="flex gap-1 flex-shrink-0">
                        <Button size="sm" variant="secondary" onClick={() => editTemplate(t)} title="Edit"><Pencil className="h-3.5 w-3.5" /></Button>
                        <Button size="sm" variant="secondary" onClick={() => { setEmailDraft({ subject: t.subject, message: t.body }); setActiveTab("email"); setNotice("Template loaded into Bulk Email."); }} title="Use in bulk email"><Send className="h-3.5 w-3.5" /></Button>
                      </div>
                    </div>
                  </div>)}
                </div>
                <p className="mt-4 text-xs text-gray-400 flex items-center gap-1.5"><Eye className="h-3.5 w-3.5" />Click the send icon on any template to load it into the Bulk Email tab.</p>
              </div>
            </div>}
          </section>
        </div>
