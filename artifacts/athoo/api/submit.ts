...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    // 1) Preferred: Resend, if API key is added in Vercel Environment Variables.
    ...
    // 2) No-key fallback: FormSubmit server-to-server.
    ...
    return res.status(502).json({ ok: false, error: 'Email service failed' });
  } catch (error) {
