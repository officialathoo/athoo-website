    ...
    <section className="relative overflow-hidden bg-[#081120] py-16 sm:py-24">
      ...
      <div className="container relative z-10 mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-2 gap-4 md:grid-cols-4 lg:gap-8">
          {stats.map((stat, i) => (
            <motion.div key={stat.label} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: false, amount: 0.25 }} transition={{ duration: 0.45, delay: i * 0.08 }} className="rounded-[2rem] border border-white/10 bg-white/5 p-5 text-center backdrop-blur sm:p-7">
